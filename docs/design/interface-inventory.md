...
| --- | --- | --- |
| Primary navigation | Five stable `CupertinoTabBar` destinations with 23-point destination icons. | Persistent 264-point sidebar with server identity, labeled destinations, and a selected-state icon treatment. |
...
| Guests | Workload/running summary; search; All/Running/Stopped filter; running-first alphabetical inventory. | Search by name, node, VMID, or kind; filter; open guest detail. |
| Nodes | Online/attention summary; search; All/Attention filter; attention-first alphabetical cards with CPU, memory, disk, cores, and uptime. | Search by node name; filter; pull/toolbar refresh. |
| Storage | Configured/shared summary; All/Shared/Local filter; type and allowed content inventory. | Filter and refresh. Utilization is never implied when the API does not report it. |
| Tasks | Recent/running/failed summary; All/Running/Failed filter; type, node, user, time, and state inventory. | Filter and refresh. |
...
production widgets. The capture pipeline produces five iPhone, five iPad, and
five Mac review images. Before release, also exercise scrolling title collapse,
